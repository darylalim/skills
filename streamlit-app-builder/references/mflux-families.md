...
5. **Sibling-org freshness (optional):** if the family's HF org is a high-velocity image-generation org worth surfacing related models from, add it to the priority-org list in `streamlit-app-builder/SKILL.md` Step 1 (the org-freshness `WebFetch` rule).
6. **No other files need editing.** SKILL.md and `CLAUDE.md` reference Apple-Silicon-only families generically as "Apple-Silicon-only families per `references/mflux-families.md` Part A" rather than enumerating them inline.
