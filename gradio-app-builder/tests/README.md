...
1. **Syntax** — `ast.parse` after substituting documented placeholders (`<org>/<model>`, `<pipeline_tag>`, `<pinned-version>`).
...
- **File-list ↔ checklist parity** — the five `### File N:` sub-headings in `SKILL.md` Step 4 match the file list in the Output checklist.

Plus Gradio-specific checks:

- **Spaces frontmatter validity** — the YAML frontmatter block in the README.md template parses; `sdk == "gradio"`, `sdk_version` looks like a semver, `app_file` is absent or `"app.py"`.
- **`gr.ChatInterface` used in T2** — T2's body contains `gr.ChatInterface(`, not `gr.Blocks(`.

## How to run

```bash
cd gradio-app-builder/tests
uv run pytest -v
```
